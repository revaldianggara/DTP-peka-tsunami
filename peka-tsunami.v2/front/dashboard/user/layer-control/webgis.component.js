(function () {
  ...
  angular.module("layerControl").controller("webgisCtrl", ["$scope", "$http", "$mdSidenav", "$timeout", "mapService_webgis", "$mdDialog", "$interval", "$filter", webgisCtrl]);

  function webgisCtrl($scope, $http, $mdSidenav, $timeout, mapService_webgis, $mdDialog, $interval, $filter) {
    // $timeout(function () {
    //         $mdSidenav('left').open();
    // });
    ...
    $scope.toggle.list2 = true;
    $scope.toggle.list3 = true;
    $scope.predDateGroup = [];
    $scope.eqDateGroup = [];
    $scope.sensorGroup = [];
    ...
    $scope.chooseModel = function (ev) {
      $mdDialog
        ...
        .then(function (response) {
          $scope.predDateGroup.filter(function (hsit) {
            mapService_webgis.removeHS("EQ_" + hsit.id);
            mapService_webgis.removeHS("predTsu_" + hsit.id);
            hsit.check = false;
          });
      ...
      $scope.predDateGroup.filter(function (hsit) {
        if (hsit.id == tid_chk) {
          if (hsit.check) {
            ...
            $http({
              ...
              function querySuccess(response) {
                if (response.data.features.length > 0) {
                  mapService_webgis.drawHS("predTsu_" + tid_chk, response.data);
            ...
            mapService_webgis.removeHS("predTsu_" + tid_chk);
            mapService_webgis.removeHS("EQ_" + tid_chk);
          }
        }
      });
    };
    $scope.eqForceItemCheck = function (tid_chk) {
      $http({
        ...
        function querySuccess(response) {
          if (response.data.features.length > 0) {
            mapService_webgis.drawHS("EQ_" + tid_chk, response.data);
          }
        },
        function queryError(response) {
          console.log("connection failed");
        ...
        if (hsit.id == tid_chk) {
          if (hsit.check) {
            ...
            $http({
              ...
              function querySuccess(response) {
                if (response.data.features.length > 0) {
                  mapService_webgis.drawHS(sensname + "_" + tid_chk, response.data);
                }
                citem.loading = false;
              },
              function queryError(response) {
            ...
            console.log("close " + tid_chk);
            citem.loading = false;
            mapService_webgis.removeHS(sensname + "_" + tid_chk);
          }
        }
      });
    };
