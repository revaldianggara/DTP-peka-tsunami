...
angular
  .module("app.nav")
  .controller("sidebarCtrl", ["$scope", "$window", "$http", sidebarCtrl])
...
function sidebarCtrl($scope, $window, $http, $interval) {
  $scope.firstTimeFlag = 0;
  $scope.category = 'dashboard';

  $scope.changeCategory = function (id) {
    $scope.firstTimeFlag = ++$scope.firstTimeFlag;
    $scope.category = id;
    $interval(function () {
      if ($window.location.reload() && $scope.firstTimeFlag > 1) {
        $scope.category = "simulasi";
      }
    }, 2000);
  };
