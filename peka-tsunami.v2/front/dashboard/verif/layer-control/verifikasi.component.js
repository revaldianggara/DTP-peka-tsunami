...
angular.module('layerControl')
        .controller('verifCtrl', ['$scope', '$http', '$mdSidenav', '$timeout', 'mapService_verif', '$mdDialog', '$mdToast', '$interval',
                verifCtrl
        ]);

function verifCtrl($scope, $http, $mdSidenav, $timeout, mapService_verif, $mdDialog, $mdToast, $interval) {
                                ...
                                if (hsit.check) {
                                        ...
                                        $http({
                                                ...
                                                if (response.data.features.length > 0) {
                                                        mapService_verif.drawHS('predTsu_' + tid_chk, response.data);
                                        ...
                                        mapService_verif.removeHS('predTsu_' + tid_chk);
                                        mapService_verif.removeHS('EQ_' + tid_chk);
                                }
                        }
                });
        }
        $scope.eqForceItemCheck = function (tid_chk) {
                $http({
                        ...
                        if (response.data.features.length > 0) {
                                mapService_verif.drawHS('EQ_' + tid_chk, response.data);
                        }
                }, function queryError(response) {
                        console.log('connection failed');
                });
                        ...
                        if (hsit.id == tid_chk) {
                                if (hsit.check) {
                                        ...
                                        $http({
                                                ...
                                                if (response.data.features.length > 0) {
                                                        mapService_verif.drawHS(sensname + '_' + tid_chk, response.data);
                                                }
                                                citem.loading = false;
                                        }, function queryError(response) {
                                                console.log('connection failed');
                                        ...
                                        console.log("close " + tid_chk);
                                        citem.loading = false;
                                        mapService_verif.removeHS(sensname + '_' + tid_chk);
                                }
                        }
                });
        }
        ...
        $scope.eqChangeTime = function (ttp) {
                ...
                $scope.eqDateGroup.filter(function (hsit) {
                        mapService_verif.removeHS('EQ_' + hsit.id);
                        hsit.check = false;
                });
                pagn2 = 0;
        ...
        $scope.tsuChangeTime = function (ttp) {
                ...
                $scope.predDateGroup.filter(function (hsit) {
                        mapService_verif.removeHS('EQ_' + hsit.id);
                        mapService_verif.removeHS('predTsu_' + hsit.id);
                        hsit.check = false;
                });
                pagn2 = 1;
