                ...
                FROM (
                    ...
                    FROM (SELECT lokasi_sensor.lsid AS lid, 
                                CASE WHEN MAX(value) - MIN(value) > 0.5 THEN 'warn'
                                    ...
                                    ELSE 'normal'
                                END AS diff,
                                MAX(sensor_val.get_time) AS ltime,
                                lokasi_sensor.coord AS coords FROM sensor_val
                            RIGHT JOIN lokasi_sensor ON sensor_val.loc_id = lokasi_sensor.lsid
                            AND sensor_val.get_time <= $2
                            AND sensor_val.get_time >= $2 - INTERVAL '5 minutes'
