<?php
	...
	if (isset($_SESSION['level']) && isset($_GET['fid']) && isset($_GET['type'])) {
	...
	$id_used = 'tid';
	if ($type_used == 'EQ') {
		$query = "SELECT feature_id.nama_feature AS parameter, input_feature.value AS value, earthquake_id.waktu AS dtm, REPLACE(REPLACE(REPLACE(ST_AsText(earthquake_id.coord, 5), 'POINT(', ''), ' ', ', '), ')', '')  AS coord FROM earthquake_id
					...
					WHERE earthquake_id.eid=$1 AND feature_id.fid IN (23,28,27,17);";
		$params = array($hsid);
	}
